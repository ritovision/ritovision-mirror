...
*Rito is a Cross-functional Authority who de-risks high-ambiguity work by leading and integrating Product, Brand, UX & Engineering into flagship experiences.*
...
![0→1 Powerhouse](https://img.shields.io/badge/0%E2%86%921%20Powerhouse-red)
...
This repository contains the source code for the **RitoVision** website.
...
CI/CD runs on GitHub Actions. The primary pipelines run on pushes to `main`, and everything can be triggered manually when needed. Workflows share composite actions in `.github/actions` to keep steps consistent.

### Pipelines

* `storybook-pipeline.yml` - lint -> vitest -> deploy Storybook.
* `mainsite-pipeline.yml` - lint -> vitest -> link check -> smoke tests -> deploy.

### Manual workflows (single-step)

* `manual-lint.yml`
* `manual-vitest.yml`
* `manual-link-check.yml`
* `manual-smoke-tests.yml`
* `manual-storybook.yml`
* `manual-mainsite.yml`

### Scheduled checks

* `link-check-scheduled.yml` - weekly production link crawl with report upload.

### Artifacts and outputs

* Link check report: `linkinator-report.md`
