...
export async function extractJsonLdScripts(page: Page): Promise<JsonLdScript[]> {
  // Evaluate all scripts in one pass to avoid per-element waits on dynamic DOM changes.
  const contents = await page
    .locator('script[type="application/ld+json"]')
    .evaluateAll(nodes => nodes.map(node => node.textContent ?? '').filter(text => text.trim().length > 0));
